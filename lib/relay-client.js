 ...
 *    "err" is undefined on success (the relay call succeeded with an HTTP
 *    202). Otherwise a VError instance is returned, optionally with a
 *    `VError.info(err).statusCode`.
 */
RelayClient.prototype.sendEvents = function (events, callback) {
    function onComplete(err, res) {
        if (err) {
            callback(new VError(err, 'failed to send events to relay'));
